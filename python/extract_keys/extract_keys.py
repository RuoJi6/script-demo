...
def find_crypto_info_in_directory(directory_path, file_extensions=None):
    ...
    patterns = {
        'crypto_keys': [
            ...
            re.compile(
                r'["\'](?:key|Key|SECRET|secret|password|Password)["\']\s*[=:]\s*["\']([a-zA-Z0-9+/=]{16,64})["\']'),
        ],
        'crypto_ivs': [
            # IV/初始向量模式
    ...
    def search_file(file_path):
        ...
        try:
            ...
            # 只返回有结果的文件
            has_results = any(
                file_results[cat] for cat in file_results if cat not in ['file', 'all_findings', 'algorithms'])
            if has_results:
                return file_results
...
def print_results(results):
    ...
    for result in results:
        ...
        if result['algorithms']:
            print(f"🔐 加密算法: {', '.join(result['algorithms'])}")
    ...
    if results:
        ...
        with open(summary_file, 'w', encoding='utf-8') as f:
            ...
            for result in results:
                ...
                total_crypto = sum(
                    len(result[cat]) for cat in result if cat not in ['file', 'all_findings', 'algorithms'])
